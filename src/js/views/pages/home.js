...
import { hideSkeletonLoader, showSkeletonLoader } from "../../global/loader.js";

// Show the skeleton loader initially
showSkeletonLoader("listings-container", 12); // Display 6 skeleton loaders

// Load the first batch of listings
loadMoreListings()
  .then(() => {
    // Hide the skeleton loader once listings are loaded
    hideSkeletonLoader("listings-container");
  })
  .catch((error) => {
    console.error("Error loading the initial listings:", error);
    hideSkeletonLoader("listings-container"); // Hide loader even if there's an error
  });
